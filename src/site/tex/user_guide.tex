...
\end{itemize}
The items shown in the list views are automatically sorted in alphabetical order.
...
\subsubsection{Project Tree View}
The Project Tree View contains elements that relate directly to a project rather than to the organisation as a whole.
Clicking on the arrows inside the tree expand/contract their respective element to show its children.
...
\begin{itemize}
  \item \menu{File > Generate > Status Report} Prompts the user for a valid save location on the local disk drive.
  \item Clicking \keys{Save} will generate the report and save it to the location set previously.
\end{itemize}

\subsection{Revert}
Revert allows a user to go back to the state where the project was last saved.
If a project has not been saved then it reverts back to the state when it was last opened.
\begin{itemize}
  \item \menu{File > Revert} Reverts the organisation back to the state when it was last saved.
