...
\subsection{Skills}
New skills can be created, edited and deleted, however there are two important skills which cannot be modified: 
\begin{enumerate}
  \item The Product Owner (PO) skill - The product owner skill is required
...
\item Dog Breeds
\end{itemize}
The estimation slider can be dragged to add/update the estimation.
Note that the estimation scale used for each story must be the same within a single backlog. Stories within a backlog
...
Acceptance criteria can be added to a story and be assigned a state of readiness to help the user in determining if the
story is ready to begin or not. The readiness can be one of three states (ready/not ready/undetermined).
\begin{itemize}
  \item To create an acceptance critera, inside the story detail pane, press the \menu{+} button under the acceptance critera table
  view.
...
new changes made after \textit{undoing} changes, will empty the \texti{redo-able} operations. Be sure to save
