...
\section{Creating a New Person}
\begin{itemize}
    \item From the menu bar, use \menu{File > New > Person}.
    \item Use the keyboard shortcut \keys{\ctrl + P}.
\end{itemize}
You will then be prompted to enter the details of the new person. 

\begin{figure}[h]
  \centering
  \includegraphics[scale=0.32]{newperson}
  \caption{The new person window.\label{newperson}}
\end{figure}

\section{Change to People List View}
\begin{itemize}
    \item From the menu bar, use \menu{View > People}.
\end{itemize}

\section{Edit a Persons Information} 
\begin{itemize}
    \item Change to the people list view.
    \item Select a person.
    \item From the menu bar, use \menu{Edit > Person}.
    \item Or right click on the person \menu{Edit}.
    \item Update the persons details.
\end{itemize}

\section{Save a Person}
People who are added to a project are automatically saved when the project is saved
