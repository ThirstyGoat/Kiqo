...
public final class DateAxis extends Axis<LocalDate> {
    ...
    /**
     * Constructs a LocalDate axis with fix lower and upper bounds.
     *
    ...
    /**
     * Constructs a LocalDate axis with a label and fix lower and upper bounds.
     *
        ...
        if (animating) {
            ...
            currentAnimationID = animator.animate(
                            new KeyFrame(Duration.ZERO,
                                            new KeyValue(currentLowerBound, oldLowerBound.toEpochDay()),
                                            new KeyValue(currentUpperBound, oldUpperBound.toEpochDay())
                            ),
                            new KeyFrame(Duration.millis(700),
                                            new KeyValue(currentLowerBound, lower.toEpochDay()),
                                            new KeyValue(currentUpperBound, upper.toEpochDay())
                            )
            );
        } else {
            currentLowerBound.set(getLowerBound().toEpochDay());
            currentUpperBound.set(getUpperBound().toEpochDay());
        }
    }
    ...
    public double getDisplayPosition(LocalDate date) {
        ...
        // The minimal LocalDate should start at the zero position, that's why we subtract it.
        double range = length - getZeroPosition();

        // Then get the difference from the actual LocalDate to the min LocalDate and divide it by the total difference.
        // We get a value between 0 and 1, if the LocalDate is within the min and max date.
        double d = (date.toEpochDay() - currentLowerBound.get()) / diff;
    ...
    public LocalDate getValueForDisplay(double displayPosition) {
        ...
        if (getSide().isVertical()) {
            // displayPosition = getHeight() - ((LocalDate - lowerBound) / diff) * range + getZero
            // LocalDate = displayPosition - getZero - getHeight())/range * diff + lowerBound
            return LocalDate.ofEpochDay((long) ((displayPosition - getZeroPosition() - getHeight()) / -range * diff
                            + currentLowerBound.get()));
        } else {
            // displayPosition = ((LocalDate - lowerBound) / diff) * range + getZero
            // LocalDate = displayPosition - getZero)/range * diff + lowerBound
    ...
    public boolean isValueOnAxis(LocalDate date) {
        return date.toEpochDay() > currentLowerBound.get() && date.toEpochDay() < currentUpperBound.get();
    }
    ...
    protected List<LocalDate> calculateTickValues(double v, Object range) {
        ...
        for (Interval interval : Interval.values()) {
            // Reset the calendar.
            calendar.setTimeInMillis(lower.toEpochDay());
            // Clear the list.
            dateList.clear();
            ...
            while (calendar.getTime().getTime() <= upper.toEpochDay()) {
                dateList.add(LocalDate.ofEpochDay(calendar.getTimeInMillis()));
            ...
            if (dateList.size() > averageTicks) {
                ...
                while (calendar.getTime().getTime() <= upper.toEpochDay()) {
                    previousDateList.add(LocalDate.ofEpochDay(calendar.getTimeInMillis()));
        ...
        // If there are at least three dates, check if the gap between the lower LocalDate and the second LocalDate is at least half the gap of the second and third date.
        ...
        if (evenDateList.size() > 2) {
            ...
            // If the second LocalDate is too near by the lower bound, remove it.
            if (secondDate.toEpochDay() - lower.toEpochDay() < (thirdDate.toEpochDay() - secondDate.toEpochDay()) / 2) {
                evenDateList.remove(secondDate);
            }

            // If difference from the upper bound to the last LocalDate is less than the half of the difference of the previous two dates,
            // we better remove the last date, as it comes to close to the upper bound.
            if (upper.toEpochDay() - lastDate.toEpochDay() < (lastDate.toEpochDay() - previousLastDate.toEpochDay()) / 2) {
    ...
    private List<LocalDate> makeDatesEven(List<LocalDate> dates, Calendar calendar) {
        ...
        if (dates.size() > 2) {
            ...
            // For each interval, modify the LocalDate slightly by a few millis, to make sure they are different days.
            // This is because Axis stores each value and won't upLocalDate the tick labels, if the value is already known.
            // This happens if you display days and then add a LocalDate many years in the future the tick label will still be displayed as day.
            for (int i = 0; i < dates.size(); i++) {
                calendar.setTimeInMillis(dates.get(i).toEpochDay());
                switch (actualInterval.interval) {
                    case Calendar.YEAR:
                        // If its not the first or last LocalDate (lower and upper bound), make the year begin with first month and let the months begin with first day.
                        ...
                        break;
                    case Calendar.MONTH:
                        // If its not the first or last LocalDate (lower and upper bound), make the months begin with first day.
                        if (i != 0 && i != dates.size() - 1) {
                            calendar.set(Calendar.DATE, 1);
                        }
                        calendar.set(Calendar.HOUR_OF_DAY, 0);
                ...
                evenDates.add(LocalDate.ofEpochDay(calendar.getTimeInMillis()));
            }
