...
#Converts a Lib.grandalf graph to a networkx graph.
#Note that the edge concept is the same, but a vertex in Lib.grandalf is called a node in networkx.
...
#Converts a networkx graph to a Lib.grandalf graph.
#Note that the edge concept is the same, but a vertex in Lib.grandalf is called a node in networkx.
def convert_nextworkx_graph_to_grandalf(G):
    from Lib.grandalf.graphs import Graph, Vertex, Edge
    V = []
    data_to_V = {}
    for x in G.nodes():
        vertex = Vertex(x)
